...
{
    return dot(a-b,c-b)/norm(a-b)/norm(c-b);
}
...
bool operator == (const plld &a, const plld &b)
{
    return (a.F == b.F && fabs(a.S-b.S) < EPS);
}
bool kmp(const vector<plld> &s, int len) 
{
    int n = s.size();
    vector<int> pi(n);
    for (int i = 1; i < n; i++) 
