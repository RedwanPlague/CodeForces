...
void dfs(const int src, vector<bool> &vis, vector<int> &keep, int &k, const LinkedList< LinkedList<int> > &tree)
{
    if(!k)
        return ;
    for(int i=0; i<tree[src].size(); i++)
    {
        ...
        {
            k--;
            keep.push_back(mp[ {min(src,v), max(src,v)} ]);
            vis[v] = 1;
            dfs(v,vis,keep,k,tree);
        }
        if(!k)
            return ;
    }
...
int main()
{
    ...
    int n, m, k; // n = number of nodes, m = number of edges
    int u, v;
    ll w;
    ...
    LinkedList< LinkedList<int> > tree(n+1);
